...
export default function StudentLoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const roleText = user?.roleText || response.data?.user?.roleText;
      const accessToken = response.accessToken || response.data?.accessToken;

      console.log("StudentLoginScreen - Response:", response);
      console.log("StudentLoginScreen - User:", user);
      console.log("StudentLoginScreen - Role:", role);
      console.log("StudentLoginScreen - RoleText:", roleText);
      console.log("StudentLoginScreen - AccessToken:", accessToken);
      console.log(
        "🔍 StudentLoginScreen - Full response:",
        JSON.stringify(response, null, 2)
      );
      console.log("👤 User object:", JSON.stringify(user, null, 2));

      if (user && accessToken) {
        const loginResult = await login(accessToken, user, role);
        console.log("StudentLoginScreen - Login result:", loginResult);
      }

      // Use utility function for consistent navigation
      navigateToHomeByRole(navigation, role, "StudentLoginScreen");
    } catch (error) {
