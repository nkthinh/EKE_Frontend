          ...
          <View style={styles.connectedTutors}>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false}>
              <View style={styles.tutorRow}>
                {(matches.length > 0 ? matches : dummyTutors).map((tutor, index) => (
                  <TouchableOpacity key={index} style={styles.tutorContainer}>
                    <Image 
                      source={tutor.image || tutor.tutorImage || require("../../assets/teacher.png")} 
                      style={styles.tutorImage} 
                    />
                    <Text style={styles.tutorName}>{tutor.name || tutor.tutorName}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </ScrollView>
            ...
            {filteredConversations.length > 0 ? (
              filteredConversations.map((message, index) => (
                <TouchableOpacity 
                  ...
                  key={index} 
                  onPress={() => navigation.navigate("StudentDetailMessageScreen", { 
                    conversationId: message.id,
                    name: message.name || message.participantName 
                  })}
                >
                  <Image 
                    source={message.image || message.participantImage || require("../../assets/teacher.png")} 
                    style={styles.messageImage} 
                  />
                  <View style={styles.messageText}>
                    <Text style={styles.messageName}>{message.name || message.participantName}</Text>
                    <Text style={styles.messagePreview}>
                      {message.preview || message.lastMessage} {message.previewTime || message.lastMessageTime}
                    </Text>
                  </View>
              ...
              dummyMessages.map((message, index) => (
                <TouchableOpacity 
                  style={styles.messageItem} 
                  key={index} 
                  onPress={() => navigation.navigate("StudentDetailMessageScreen", { name: message.name })}
                >
